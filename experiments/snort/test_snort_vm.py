...
class TestSnortVm(TestSnortBase):

	def __init__(self, args, container_name='snort'):
		super().__init__()
	...
	def probe_vm_ip(self, vm_name):
		while True:
			...
			for line in ret.output.decode('utf-8').split('\n'):
				...
				if vm_name in line:
					# "2016-04-12 01:13:28  52:54:00:fb:44:5b  ipv4      192.168.122.204/24        snort-vm     -"
	...
	def run(self):
		...
		self.snort_proc = self.shell.spawn([RUNNER_TMPDIR + '/tester_script/' + self.args.vm_name + '.py',
			self.args.vm_ip, '/tmp/test', self.args.vm_nic, str(self.args.swappiness), str(self.args.interval), '/var/log/snort'],
			cwd=self.session_tmpdir, stdout=sys.stdout.buffer, stderr=sys.stdout.buffer, store_pid=True, allow_error=True)
		self.wait_for_snort('/var/log/snort', prepend=['ssh', 'root@' + self.args.vm_ip])
		...
		log('Snort VM script returned with value %d.' % snort_result.return_code)
		del self.snort_proc
	...
	def postwork(self):
		...
		if self.status == self.STATUS_DONE:
			self.simple_call(['rsync', '-zvrpE', 'root@%s:%s/*' % (self.args.vm_ip, '/var/log/snort'), self.session_tmpdir + '/'])
...
def main():
	parser = argparse.ArgumentParser(description='Run Snort inside a virtual machine (managed by libvirt) on remote host and collect system info.')
	...
	parser.add_argument('--vm-name', '-n', nargs='?', type=str, default='snort-vm', help='Name of the virtual machine registered to libvirt.')
