...
def monitor_system_res(delay_sec, suffix):
	print('System monitor started.', file=sys.stderr)
	base_timestamp = int(time.time())
	ncores = psutil.cpu_count()
	with open('sysstat' + suffix + '.csv', 'w') as f:
		...
		while proceed:
			...
			rel_timestamp = timestamp - base_timestamp
			...
			line = str(timestamp) + ', ' + str(rel_timestamp) + ', ' + str(ncores) + ', ' + str(total_cpu_percent) + ', '
...
def monitor_nerwork_usage(delay_sec, nics, suffix):
	"""
	...
	nics = {k: open('netstat.%s' + suffix + '.csv' % k, 'w') for k in nics.split(',') if k in all_nics}
	...
	while proceed:
		...
		for nic, f in nics.items():
			...
			f.write(str(timestamp) + ', ' + str(rel_timestamp) + ', ' + nic + ', ' + \
...
def main():
	try:
		...
		parser.add_argument('--suffix', '-s', type=str, nargs='?', default='', required=False, help='String to add to the end of file name before ext name. Default: "".')
		args = parser.parse_args()
		sysstat_thread = threading.Thread(target=monitor_system_res, args=(args.delay, args.suffix))
		sysstat_thread.start()
		if args.nic is not None:
			netstat_thread = threading.Thread(target=monitor_nerwork_usage, args=(args.delay, args.nic, args.suffix))
