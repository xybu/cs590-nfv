...
class TestSnortDocker(TestSnortBase):
	...
	def run(self):
		...
		self.sysmon_proc = self.shell.spawn([RUNNER_TMPDIR + '/tester_script/sysmon.py',
			...
			'--enable-ps', '--ps-keywords', 'snort', 'docker', '--ps-outfile', 'psstat.docker.csv'],
		...
		self.snort_proc = self.shell.spawn(['docker', 'run', '-i', '--name', self.container_name, '--cpuset-cpus', self.args.cpuset,
			'--memory', self.args.memory, '--memory-swappiness', str(self.args.swappiness), '--net=host',
			'-v', '%s:%s' % (self.session_tmpdir, '/var/log/snort'), 'xybu:snort', 'snort', '-A', 'fast', '-c', '/etc/snort/snort.conf', '-l', '/var/log/snort', '-i', dest_nic],
		...
		log('Snort container returned with value %d.' % snort_result.return_code)
...
def main():
	parser = argparse.ArgumentParser(description='Run Snort inside a Docker container on remote host and collect system info.')
